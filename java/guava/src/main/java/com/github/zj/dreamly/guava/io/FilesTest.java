 ...
 *
 * @author: 苍海之南
 * @since: 2019-10-29 11:33
 **/
public class FilesTest {
	private final String filePath = FilesTest.class.getResource("/").getPath();
	private final String SOURCE_FILE = filePath + "/io/source.txt";
	private final String TARGET_FILE = filePath + "/io/target.txt";
	...
	public void testCopyFileWithJDKNio2() throws IOException {
		java.nio.file.Files.copy(
			Paths.get(filePath, "io", "source.txt"),
			Paths.get(filePath, "io", "target.txt"),
	...
	public void testFileWrite() throws IOException {
		String testPath = filePath + "/io/testFileWrite.txt";
	...
	public void testFileAppend() throws IOException {
		String testPath = filePath + "/io/testFileAppend.txt";
		File testFile = new File(testPath);
	...
	public void testTouchFile() throws IOException {
		File touchFile = new File(filePath + "/io/touch.txt");
	...
	public void testRecursive() {
		...
		this.recursiveList(new File(filePath), list);
...
//        FluentIterable<File> files = Files.fileTreeTraverser().preOrderTraversal(root);
		final Iterable<File> files = Files.fileTraverser().depthFirstPreOrder(root);
	...
	public void testTreeFilesBreadthFirstTraversal() {
		File root = new File(filePath);
		final Iterable<File> files = Files.fileTraverser().breadthFirst(root);
		Stream.of(files).forEach(System.out::println);
	}
